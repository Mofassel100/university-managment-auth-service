export const academicFacultyFilteringTermFiles = [
  'searchTerm',
  'title',
  'syncId',
];
export const academicFacultySeachTermFiles = ['title'];
export const EVENT_ACADEMIC_FACULTY_CREATED = 'academic-faculty.created';
export const EVENT_ACADEMIC_FACULTY_UPDATED = 'academic-facult.updated';
export const EVENT_ACADEMIC_FACULTY_GET_ALL = 'academic-faculty.get-all';
export const EVENT_ACADEMIC_FACULTY_GET_SINGLE = 'academic-faculty.get-single';
export const EVENT_ACADEMIC_FACULTY_GET_DELETED = 'academic-faculty.deleted';
